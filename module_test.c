...
int gva2hpa(unsigned long gva, unsigned long hva, int pid){
		...
		if(!my_pmd_alloc(mm, hva_pud, hva)){
			...
			if(!my_pte_alloc(mm, hva_pmd)){
				...
				if(!pte_present(*hva_pte)){
					pte_t * tmp = (pte_t *)get_zeroed_page(GFP_PGTABLE_KERNEL);
					set_pte(hva_pte, __pte(PROT_NORMAL| PTE_USER| PTE_VALID| PTE_TABLE_BIT | __pa(tmp)));
				}
				/*printk("HVA PGD Good: %llx\n", pgd_val(*hva_pgd));
				printk("HVA PUD Good: %llx\n", pud_val(*hva_pud));
				printk("HVA PMD Good: %llx\n", pmd_val(*hva_pmd));
				printk("HVA PTE Good: %llx\n", pte_val(*hva_pte));*/		
			}			
		}
	}
	...
	if(!my_pud_alloc(mm, &__p4d(pgd_val(*gva_pgd)), gva)){
		if(!pte_present(pgd_pte(*gva_pgd))){
			pgd_t * tmp = (pgd_t *)get_zeroed_page(GFP_PGTABLE_USER);
			*gva_pgd = __pgd(PROT_NORMAL| PTE_USER| PTE_VALID| PTE_TABLE_BIT | __pa(tmp));
		}
		...
		if(!my_pmd_alloc(mm, gva_pud, gva)){
			...
			if(!my_pte_alloc(mm, gva_pmd)){
				...
				/*printk("CR3 Good: %llx\n", pgd_val(*(mm->pgd)));
				printk("GVA PGD Good: %llx\n", pgd_val(*gva_pgd));
				printk("GVA PUD Good: %llx\n", pud_val(*gva_pud));
				printk("GVA PMD Good: %llx\n", pmd_val(*gva_pmd));
				printk("GVA PTE Good: %llx\n", pte_val(*gva_pte));*/
				set_pte(gva_pte, *hva_pte);
				gva_pte->pte |= PTE_USER; gva_pte->pte |= PROT_NORMAL;
			}			
		}
	}
	return 1;
